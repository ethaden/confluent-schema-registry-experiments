...
public class ProducerV1 {
    ...
    void sendAvroProducer(int nb) {
        ...
        try (KafkaProducer<String, ExampleEventRecord1> producer = new KafkaProducer<>(avroSettings())) {
            for (int i=0; i < nb; i++) {
                String key = Integer.toString(count);
                ExampleEventRecord1 value = ExampleEventRecord1.newBuilder()
                        .build();
                ProducerRecord<String, ExampleEventRecord1> producerRecord = new ProducerRecord<>(TOPIC, key, value);
